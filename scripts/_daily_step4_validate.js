...
function checkScripts(html, fname) {
  ...
  if (errors.length === 0) ok(fname + ': all ' + count + ' script blocks parse (new Function)');
}
...
const newCh = CHAPTERS.find(c => c.id === 22);
if (!newCh) fail('New chapter 22 missing');
else if (newCh.lessons.join(',') !== '101,102') fail('New chapter 22 lessons mismatch', JSON.stringify(newCh.lessons));
else ok('New chapter 22 present with lessons [101,102]');
...
if (TRAIL_GAMES[101] && TRAIL_GAMES[102]) ok('TRAIL_GAMES covers new ids 101, 102 (' + TRAIL_GAMES[101] + ', ' + TRAIL_GAMES[102] + ')');
...
// 8. Title uniqueness (no repeated theme)
...
else ok('No duplicate lesson titles');
