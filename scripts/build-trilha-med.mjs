#!/usr/bin/env node
/**
 * build-trilha-med.mjs — monta lessons_med_data.js e valida o schema.
 *
 *   node scripts/build-trilha-med.mjs
 *
 * Fontes: scripts/med-trilha-a.json (701-706) e -b.json (707-712).
 ...
 * O aluno é médico, mas está aprendendo INGLÊS, não glossário. E aqui há uma
 * armadilha extra: quase todo termo técnico médico é cognato (diagnosis,
 * symptom, hypertension), então marcá-lo como palavra ensinada seria inflar o
 * curso com o que o aluno já sabe. O lado `clinico` é reservado ao que NÃO é
 * cognato — rash, sore, dizzy, lump, numb, swollen. Cada palavra carrega um
 * campo `tema`: "geral" ou "clinico". O construtor exige
...
const licoes = [
    ...JSON.parse(fs.readFileSync(path.join(RAIZ, 'scripts/med-trilha-a.json'), 'utf8')),
    ...JSON.parse(fs.readFileSync(path.join(RAIZ, 'scripts/med-trilha-b.json'), 'utf8')),
];
...
for (const l of licoes) {
    ...
    for (const v of l.vocab || []) {
        ...
        if (v.tema !== 'geral' && v.tema !== 'clinico') { erro(id, `"${v.en}" sem tema "geral"/"clinico"`); continue; }
        v.tema === 'clinico' ? a++ : g++;
    ...
    if (tot && (g / tot > 0.7 || a / tot > 0.7))
        erro(id, `mix desequilibrado: ${g} geral / ${a} clínico (nenhum lado pode passar de 70%)`);
...
const totalVocab = nGeral + nClin;
const pctClin = totalVocab ? nClin / totalVocab : 0;
if (totalVocab && (pctClin < 0.40 || pctClin > 0.60))
    erro('geral', `mix do curso: ${nGeral} geral / ${nClin} clínico (${(pctClin * 100).toFixed(0)}% clínico) — o alvo é 50%, tolerância 40-60%`);
...
const destino = path.join(RAIZ, 'lessons_med_data.js');
...
const carregadas = sandbox.window.LESSONS_MED;

console.log(`lessons_med_data.js — ${(fs.statSync(destino).size / 1024).toFixed(0)}KB`);
...
console.log(`  ✓ window.LESSONS_MED exposta (é assim que o build enxerga)`);
console.log(`  ✓ schema completo: 4 verbs · 30 vocab · 4 expressions · 8 sentences · grammar · 5 quiz (com why) · 5 speak`);
console.log(`  ✓ mix: ${nGeral} geral / ${nClin} clínico (${(pctClin * 100).toFixed(0)}% clínico)`);
